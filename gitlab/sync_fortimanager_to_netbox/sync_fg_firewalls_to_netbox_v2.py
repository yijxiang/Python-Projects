"""
v2 Adapted to run in Gitlab Pipline
Sychronize FortiManger with Netbox
Created by Louis Uylaki 06/2021
"""
import logging, sys, pynetbox, requests, argparse
...
parser=argparse.ArgumentParser(description="FortiManager to Netbox Args")
parser.add_argument('--nb_token', required=True, help="Netbox Token")
parser.add_argument('--nb_url', required=True, help="Netbox URL")
parser.add_argument('--prd_fmg_un', required=True, help="FortiManager Username")
parser.add_argument('--prd_fmg_pw', required=True, help="FortiManager Password")
parser.add_argument('--prd_fmg_host', required=True, help="Fortimanager Address")

args = parser.parse_args()
prd_fmg_un = args.prd_fmg_un
prd_fmg_pw = args.prd_fmg_pw
prd_fmg_host = args.prd_fmg_host
...
if get_all_firewalls[0] == 0:
    for fw in tqdm(get_all_firewalls[1], desc="Sync Firewalls With Netbox"):
        if fw['conn_status'] == 2:
                        ...
                        if ip_address != '0.0.0.0':
                            ...
                            if fw_ip == ip_address: # check if IP is the VRRP or if it is Primary Management for non-redundant firewalls
                                ...
                                if get_vrrp_ip.role == None and fw['ha_slave'] != None:
                                    get_vrrp_ip.role = 41 # Static value for Netbox dtnaacvdl210 ip role id
                                ...
                                if vrrp_role_id != 41 and fw['ha_slave'] != None:
                                    get_vrrp_ip.role = 41 # 41 is Static value for Netbox dtnaacvdl210 ip role id of VRRP
                                ...
                                if fw['ha_slave'] != None:  # If FW IS redundant
                                    ...
                                    if primary_fw_name == "FWTLXE16-DCN-01":    # Add special case for FWTLXE16-DCN-01,
                                        octets[3] = str(int(octets[3])-2)        # it doesn't follow standards used by all other firewalls
                                    else:
                ...
                if get_nb_ha_fw.device_type.manufacturer.id == 3: # If FW is a Fortinet in Netbox
                    ...
                    if ha_fw_primary_nb_ip != ha_fw_cidr_ip:
                        ...
                        try:
                            ...
                            logger.error(f"\"{ha_fw_primary_ip}\", {ve}")
                            errors.append(f"\"{ha_fw_primary_ip}\", {ve}")
                            continue

                        if get_ha_ip == None:
                            logger.info(f"Primary IP \"{ha_fw_primary_ip}\" Not In Netbox.")
